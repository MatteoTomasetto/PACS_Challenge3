...
inline void
Matrix<Scalar, Order>::Reader(char* filename)
{
	int ret_code;
    MM_typecode matcode;
    FILE *f;
    ...
    if ((ret_code = mm_read_mtx_crd_size(f, &static_cast<long unsigned int>(nRows),
    										&static_cast<long unsigned int>(nCols), &nz)) !=0)
    ...
    for (i=0; i<nz; i++)
    {
        ...
        ncData.insert( std::pair( std::tie(I[i], J[i]), val[i]))
    }
    if (f !=stdin) fclose(f);
}
...
template<typename Scalar, StorageOrder Order>
template<class VectorInputType> //, enable_if_v< std::is_convertible<VectorInputType::value_type, Scalar> > >
